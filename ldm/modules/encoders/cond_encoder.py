...
class Uformer_4channel(nn.Module):
    def __init__(self, img_size=128, in_chans=3,out_chans=3,
                 embed_dim=32, depths=[2, 2, 2, 2, 2, 2, 2, 2, 2], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
                 win_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
        ...
        # Input/Output
        self.input_proj = InputProj(in_channel=in_chans*4, out_channel=embed_dim, kernel_size=3, stride=1,
                                    act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj(in_channel=2 * embed_dim, out_channel=out_chans*4, kernel_size=3, stride=1)
...
if __name__ == "__main__":
    # arch = Uformer_4channel
    # input_size = 128
    # # arch = Uformer_Cross
    # depths = [1, 2, 8, 8, 2, 8, 8, 2, 1]
    # # model_restoration = UNet(dim=32)
    # model_restoration = arch(img_size=input_size, embed_dim=32, depths=depths,in_chans=1,
    ...
    # x = x.to(device)
    # x = model_restoration.to(device)(x)
    # print(x.shape)
    # ops, params = get_model_complexity_info(model_restoration, (1,512,512), as_strings=True, print_per_layer_stat=True, verbose=True)
    # print(ops, params)
    # print("number of GFLOPs: %.2f G" % (model_restoration.flops() / 1e9))
    # print('\nTrainable parameters : {:.2f} M\n'.format(sum(p.numel()/1e6 for p in model_restoration.parameters() if p.requires_grad)))
    img = torch.randn(8, 3, 256, 256)
    model = Uformer_4channel(img_size=img.shape[2], embed_dim=32, depths=[1, 2, 8, 8, 2, 8, 8, 2, 1], in_chans=3,
                             win_size=8, mlp_ratio=4., qkv_bias=True,
                             token_projection='linear', token_mlp='leff',
                             downsample=Downsample, upsample=Upsample, se_layer=False)
    x = model(img)
