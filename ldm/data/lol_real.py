...
class LOLDataset(Dataset):
    def __init__(self, data_dir, mode='train'):
        ...
        self.path_real = os.path.join(self.root_dir, 'Train')
        self.path_syn = os.path.join(self.root_dir, 'syn')
        self.path_eval = os.path.join(self.root_dir, 'Test')
        if mode == "train":
            self.high_names = glob(self.path_real + '/Normal/*.png')  # + glob(self.path_syn + '/high/*.png')
            ...
            self.high_names = glob(self.path_eval + '/Normal/*.png')
...
class LOLDatasetTrain(LOLDataset):
    def __init__(self, **kwargs):
        super().__init__(data_dir='LOL-v2/Real_captured', mode='train', **kwargs)


class LOLDatasetVal(LOLDataset):
    def __init__(self, flip_p=0., **kwargs):
        super().__init__(data_dir='LOL-v2/Real_captured', mode='eval', **kwargs)
